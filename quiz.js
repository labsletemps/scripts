(function ($) {
  ...
  var buildQuiz = function (input) {
    ...
    $(".quiz-container").html("<div class='progress'>Question " + qnumber + "&nbsp;sur&nbsp;" + input.length + "</div><div class='qq-question'><div class='qq-description'>" + input[currentQuestion].description + "</div><br><div class='question'>" + input[currentQuestion].question + "</div></div>" +
      ...
      "<button class='qq-button hint'>Un indice ?</button>" +
      "<button class='qq-button submit-answer'>Votre réponse</button>" +
  ...
  var checkAnswer = function () {
    if ($(".selected").length > 0) {
      ...
      if (answer == input[currentQuestion].answer) {
        ...
        score++;
        displayProgress();
        $(".answer").html("<p>Exact !</p><p>" + input[currentQuestion].correct + "</p>");

      } else {
        trackEvent(
          'q' + qnumber + '-answered-incorrectly',
          'Q' + qnumber + ' answered incorrectly');
        $(".answer").html("<p>Désolé !</p><p> " + input[currentQuestion].incorrect + "&nbsp;La réponse exacte est" + input[currentQuestion].answer + ".</p>");
      }
      if (currentQuestion != (input.length-1)) {
        $(".answer").append("<button class='qq-button next'>Suivant</button>");
        $('.next').on('click', nextQuestion);
      } else {
        $(".answer").append("<button class='qq-button check-score'>Voir votre score</button>");
  ...
  var finalScore = function () {
    ...
    $(".quiz-container")
      .html("<div class='scorecard'><p>Vous avez répondu à </p><p>" + score + "&nbsp;réponses sur &nbsp;" + input.length + "</p><div id='social-media'><ul><li><a class=\"fb-share\" href='http://www.facebook.com/sharer.php?u=" + link + "' target='_blank'>" + facebook + "</a></li><li><a class=\"twitter-share\" href='http://twitter.com/home?status=I scored " + score + "/" + input.length + " on this quiz " + link + " via @dataletemps" + "' target='_blank'>" + twitter   + "</a></li><li><a class=\"gplus-share\" href='https://plus.google.com/share?url=" + link + "' target='_blank'>" + google + "</a></li></ul></div></div>");
